...
**Data backups run every day; retention is 90 days.**

(Backups include database, media, static files, code, env, and SSL certs as done by the server backup script.)

## How it works

- **Schedule:** The scheduler runs once per day from cron (e.g. 2 AM). Each run executes the server backup script and then applies retention.
- **Retention:** Backups older than **90 days** are deleted automatically (backup directories, `.tar.gz` archives, and log files in `/root/chip_3_backups/`).
...
This installs `/root/backup_chip3_scheduled.sh` and adds a cron job to run it daily at 2 AM.
...
- Remove or disable the retention/cleanup inside `/root/backup_chip3.sh`, so only this scheduler’s 90-day retention runs, or  
- Change that script to keep backups for 90 days instead of 7.

The scheduler script deletes in `/root/chip_3_backups/` anything older than 90 days (directories `backup_*`, files `backup_*.tar.gz`, `backup_log_*.txt`).
